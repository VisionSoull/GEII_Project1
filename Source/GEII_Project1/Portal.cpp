...
APortal::APortal()
{
	...
	// Set relative rotation for correct orientation of the portal and default scale size
	PortalMesh->SetRelativeRotation(FRotator(0.f, -90.f, 90.f));
	PortalMesh->SetRelativeScale3D(FVector(2.2f, 2.2f, 2.2f));

	// Enable override custom near clipping plane on the scene capture component 2D
	PortalCamera->bOverride_CustomNearClippingPlane= true;
	UE_LOG(LogTemp, Warning, TEXT("OverrideCustomNearClippingPlane is set to: %s"), (PortalCamera->bOverride_CustomNearClippingPlane ? TEXT("True") : TEXT("False")));
}
